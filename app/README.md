...
由项目作者执行测试、类型检查、构建、浏览器验收，以及 Git 提交、打标签和推送。代码协作者负责修改实现与 README、同步版本号并提供建议 commit 信息，不代运行上述操作。本次挥手与白色手臂改动待作者验收。
...
Git 标签 `v0.1.0` 保留最初的第三人称原型；之后的提交记录第一人称修正。`0.3.0` 已同步到 `package.json` 与 `package-lock.json`；本轮未代执行 Git 提交、打标签或推送。
